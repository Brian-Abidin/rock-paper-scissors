...
function playRound(playerSelection, computerSelection) {
  // create function takes 2 parameters of user input RPS vs Computer RPS
  const round = document.querySelector("#round");
  while (playerSelection) {
    if (win === 5 || loss === 5) break;
    else {
      if (playerSelection === "Rock" && computerSelection === "Paper") {
        // create if statements comparing all outcomes of RPS for computer and User
        loss += 1;
        document.getElementById("loss").textContent = loss;
        if (loss === 5) break;
        round.textContent =
          "You Lose! " + computerSelection + " beats " + playerSelection + "!"; // return the winner using a string "win/lose x beats y"
